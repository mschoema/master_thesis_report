...
Despite all this previous work, most research ideas still have to be implemented into a large open-source database to be available for everyone to use. \textit{MobilityDB} \note{link to mobilitydb} aimes to solve exactly that. It is an open-source extension on top of Postgres \note{link to postgres} and PostGIS \note{link to postgis} that adds spatiotemporal types for moving points as well as temporal types for moving booleans, integers, reals and strings. Implementation of moving region concepts into MobilityDB still has to be done, and this is the topic of this Master thesis.
...
\begin{itemize}
    ...
    \item Moving regions can be categorised into \textit{deforming} regions and \textit{non-deforming} regions. The first paper making this distinction uses the term \textit{fixed-shape} to denote these non-deforming regions. Based on this paper, other authors \note{example papers: modeling and representing, A polyhedra based model, ...} also started to use this terminology, and we will thus do the same.
